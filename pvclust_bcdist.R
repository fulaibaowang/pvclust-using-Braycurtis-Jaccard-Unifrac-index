...
{
  if(is.null(main))
    main="Cluster dendrogram with AU values (%)"
...
text.pvclust <- function(x, col=c(2,3,8), print.num=TRUE,  float=0.01, cex=NULL, font=NULL, ...)
{
  ...
  a <- text(x=axes[,1], y=axes[,2] + float * wid, au,
            col=col[1], pos=1, offset=.3, cex=cex, font=font)
...
dist.pvclust <- function(x, method="euclidean", use.cor="pairwise.complete.obs")
{
  ...
  #add braycurtis method
  else if(!is.na(pmatch(method,"bray"))) {
	try(library("ecodist"), install.packages("ecodist"))
	bcdist(t(x)) # Niels start
  }
  #add jaccard method
    else if(!is.na(pmatch(method,"jaccard"))) {
	try(library("vegan"), install.packages("vegan"))
	vegdist(t(x), method="jaccard") 
  }
  #add unifracs unweighted
   else if(!is.na(pmatch(method,"unweighted"))) {
	try(library("GUniFrac"), install.packages("GUniFrac"))
	as.dist(GUniFrac(t(x), tree, alpha = c(0, 0.5, 1))$unifracs[, , "d_UW"])
	  }
	  #add unifracs weighted
	else if(!is.na(pmatch(method,"weighted"))) {
	try(library("GUniFrac"), install.packages("GUniFrac"))
	as.dist(GUniFrac(t(x), tree, alpha = c(0, 0.5, 1))$unifracs[, , "d_1"])
	  }
   else
    dist(t(x),method)
}
